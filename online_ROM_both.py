...
POD_mat_p = np.loadtxt(filename2)
dofP,throwaway = np.shape(POD_mat_p)

POD_mat_p=POD_mat_p[:,:val_POD_p]

nRomP = val_POD_p

print("The total number of pressure basis functions:" + str(nRomP))

POD_funcsP = [Function(Q) for i in range(0,nRomP)]
for i in range(0,nRomP):
    POD_funcsP[i].vector()[:] = np.array(np.real(POD_mat_p[:,i])) #Copy POD vectors into Fenics Functions
...
#Load in Supremizers
# filename3 = './POD_vecs/SupremizerMatBE.txt'
# Sup_mat = np.loadtxt(filename3)
Sup_mat,inf_sup_const = offline_supremizer_stabalization(N,N_outer,N_inner,nRomP,dofP,POD_mat_p)
...
bv = np.zeros((nRomV,1))
bvf = np.zeros((nRomV,1))
xv = np.zeros((nRomV,1)) #un in ROM basis
xvM1 = np.zeros((nRomV,1))
#Pressure
...
#Norm
N = np.zeros((nRomP,nRomP))

print("Now Doing the Assembly")

#Assemble the offline systems
for i in range(0,nRomV):
	bvf[i]= assemble(inner(POD_funcsV[i],f)*dx)
	for j in range(0,nRomP):
		K_p[i,j] = nu*assemble(dot(curl(POD_funcsV[i]),cross_2(n,grad(POD_funcsP[j])))*ds) 
		A_s[i,j] = nu*assemble(a_1(POD_funcsV[i],Sup_funcs[j])*dx) #Supremizer nu/contract term
		D2_s[i,j] = assemble(inner(POD_funcsV[i],Sup_funcs[j])*dx) #Supremizer time discretization term
	for j in range(0,nRomV):
		H_V[i,j] = assemble((1./dt)*inner(POD_funcsV[i],POD_funcsV[j])*dx + nu*a_1(POD_funcsV[i],POD_funcsV[j])*dx) #LHS Matrix for Velocity
		D[i,j] = assemble((1./dt)*inner(POD_funcsV[i],POD_funcsV[j])*dx) #RHS for Velocity--time disc
		for k in range(0,nRomV):
			T[k,i,j]= T[k,i,j]+assemble(b(POD_funcsV[k],POD_funcsV[j],POD_funcsV[i])*dx) #LHS matrix Velocity(nonlinear)
...
for i in range(0,nRomP):
	...
	for j in range(0,nRomP):
		...
		H_PP[i,j] = assemble(a_1(POD_funcsP[i],POD_funcsP[j])*dx) #RHS for pressure matrix (never changes)
		N[i,j] = assemble(inner(POD_funcsP[i],POD_funcsP[j])*dx)
...
angle_mat = np.zeros((nRomV,nRomP))

for i in range(0,nRomV):
	for j in range(0,nRomV):
		angle_mat[i,j] = assemble(inner((POD_funcsV[i]),(Sup_funcs[j]))*dx)


U_princ, alpha_val, V_princ = np.linalg.svd(angle_mat, full_matrices=False)
print("The value for alpha is:" + str(alpha_val[0]))


time2 = np.zeros(t_num)
t = t + dt
time2[0] = t
u0.assign(POD_v_proj)



xv = coeff_holdV #Coefficents for u0

for jj in range(0,t_num-1):
	t = t + dt
	time2[jj+1] = t
	print('Numerical Time Level: t = '+ str(t))

	#Set up Velocity Systems
	bv = np.zeros((nRomV,1))
	...
	for i in range(0,nRomV):
		...
		for j in range(0,nRomV):
			...
			for k in range(0,nRomV):
				C[i,j] = C[i,j]+xv[k]*T[k,i,j]
	Av = H_V + C
	#Solver for velocity
	luV,piV = scipy.linalg.lu_factor(Av) #LU decomposition
	#Copy over solutions
	xvM1 = xv.copy() #Previous timestep xvM1 is un coefficents
	xv = scipy.linalg.lu_solve((luV,piV),bv) #xv is now un+1 coefficents

	# sol_V = POD_mat_v.dot(xv) #Project back onto full basis
	# un.vector()[:] = np.array(np.real(sol_V[:,0])) #Update u0 so it's now what it should be (stays un thing)
	...
	for i in range(0,nRomP):
		...
		for j in range(0,nRomV):
			...
			for k in range(0,nRomV):
				...
				bp_p[i] = bp_p[i]- xv[j]*xv[k]*T2_p[j,k,i]

	#### Test for preassemble/nonpreassambled equality
	# bp_p_2 = np.zeros((nRomP,1))

	# for i in range(0,nRomP):
	# 	bp_p_2[i]= assemble(-convect(un,un,grad(POD_funcsP[i]))*dx + \
	# 		inner(f,grad(POD_funcsP[i]))*dx+nu*dot(curl(un),cross_2(n,grad(POD_funcsP[i])))*ds)
	# print(np.linalg.norm(bp_p-bp_p_2))	

	# bp_s_2 = np.zeros((nRomP,1))
	# for j in range(0,nRomP):
	# 	bp_s_2[j] = assemble((1./dt)*inner(un-u0,Sup_funcs[j])*dx + b(u0,un,Sup_funcs[j])*dx -inner(f,Sup_funcs[j])*dx)

	# print(np.linalg.norm(bp_s_2 - bp_s))

	xp_s = scipy.linalg.lu_solve((luP_s,piP_s),bp_s)
	# sol_P_s = POD_mat_p.dot(xp_s) #Actual solution (times functions basically)
	# pn_s.vector()[:] = np.array(np.real(sol_P_s[:,0])) 		

	xp_p = scipy.linalg.lu_solve((luP_p,piP_p),bp_p)
	# sol_P_p = POD_mat_p.dot(xp_p) #Actual solution (times functions basically)
	# pn_p.vector()[:] = np.array(np.real(sol_P_p[:,0])) 		
	pnorm_s = 0.
	pnorm_p = 0.
	...
	for j in range(0,nRomV):
		...
		for k in range(0,nRomV):
			...
			pnorm_s = xp_s[k]*xp_s[j]*N[k,j]
			pnorm_p = xp_p[k]*xp_p[j]*N[k,j]
	for i in range(0,nRomP):
	...
	press_norm_pp[jj] = np.sqrt(pnorm_p)
	press_norm_s[jj] = np.sqrt(pnorm_s)
	...
	x0 = xv #Assign coefficents of previous time step
...
np.savetxt('array_folder/time_arr.txt',time2)
