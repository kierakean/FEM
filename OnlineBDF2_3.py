...
velocity_paraview_file = File("para_plotting/velocity_solution_div_disc.pvd")
...
unM1.vector()[:] = np.array((initial_conV[:]))

#### Define Functions For initial conditions round 2
un = Function(X)
#Load in initial conditions
initial_conV = np.loadtxt('./POD_vecs/velocity2_init.txt') #Load in intitial condition in this example we chose not to start from T = 0
#Copy over initial conditions
...
#Declare matricies to store POD left and right hand side and solution
...
xvnM1= np.zeros((nRomV,1)) #unM1 in ROM basis

#Assemble the offline systems
for i in range(0,nRomV):
	bv1[i]= assemble(inner(POD_funcsV[i],f)*dx)
	for j in range(0,nRomV):
		H_V[i,j] = assemble((3.0/(2.*dt))*inner(POD_funcsV[i],POD_funcsV[j])*dx + nu*a_1(POD_funcsV[i],POD_funcsV[j])*dx) #LHS Matrix
		D[i,j] = assemble((1./dt)*inner(POD_funcsV[i],POD_funcsV[j])*dx)
		for k in range(0,nRomV):
			T[k,i,j]= T[k,i,j]+assemble(b(POD_funcsV[k],POD_funcsV[j],POD_funcsV[i])*dx) #LHS matrix (nonlinear)

frameNum = 50 #(50 per second)
frameRat = int(1/(framNum*dt))
...
for jj in range(0,t_num-1):
	...
	if jj < 2: ##Messy way to make initial conditions work
		Av = np.zeros((nRomV,nRomV))
		C = np.zeros((nRomV,nRomV))
		for i in range(0,nRomV):
			for j in range(0,nRomV):
				for k in range(0,nRomV):
					C[i,j] = C[i,j]+(2*xv[k]-xvnM1[k])*T[k,i,j]
		Av = H_V+C
		luV,piV = scipy.linalg.lu_factor(Av) #LU decomposition
		for i in range(0,nRomV):
			bv[i] = assemble(inner(POD_funcsV[i],f)*dx + (2.0/dt)*inner(un,POD_funcsV[i])*dx -(1.0/(2.*dt))*inner(unM1,POD_funcsV[i])*dx )
	...
	#Velocity Matrix (and finish the right hand side to minimize number of loops, might not be necessary)
	if jj >= 2:
		Av = np.zeros((nRomV,nRomV))
		C = np.zeros((nRomV,nRomV))
		bv = bv1.copy()
		for i in range(0,nRomV):
			for j in range(0,nRomV):
				bv[i] = bv[i] + (2*xv[j]-(1./2.)*xvnM1[j])*D[i,j] 
				for k in range(0,nRomV):
					C[i,j] = C[i,j]+(2*xv[k]-xvnM1[k])*T[k,i,j] #The nonlinear thing
		Av = H_V+C

		luV,piV = scipy.linalg.lu_factor(Av) #LU decomposition

		xvnM1 = xv.copy()
		xv = scipy.linalg.lu_solve((luV,piV),bv) #un in ROM basis
